...
If no persona doc exists for the named colleague, the skill stops and points the user at the `slack-distillation` skill. `ask-colleague` does **not** create or refresh personas — it only reads them.
...
To create or refresh a persona, use the **`slack-distillation`** skill — it dumps a colleague's Slack history and distills it into the persona doc this skill consumes.
...
- A persona is only as current as the last `slack-distillation` run.
